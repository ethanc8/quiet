...
const PREFIX = 'MentionDropdown'

const classes = {
  MentionDropdown: `${PREFIX}__MentionDropdown`,
  MentionDropdownItem: `${PREFIX}__MentionDropdownItem`,
  selectedItem: `${PREFIX}__selectedItem`,
}

const StyledRoot = styled('div')(({ theme }) => ({
  [`&.${classes.MentionDropdown}`]: {
    maxHeight: '200px',
    width: '100%',
...
export interface MentionDropdownProps {
  ...
  onClickAway: () => void
  onMentionableSelect: (username: string) => void
}
...
export const MentionDropdown: React.FC<MentionDropdownProps> = ({
  ...
  return (
    <ClickAwayListener onClickAway={onClickAway}>
      <StyledRoot
        ref={dropdownRef}
        className={classes.MentionDropdown}
      ...
      >
        {suggestions.slice(0, MAX_MENTION_SUGGESTIONS).map((suggestion, index) => (
          <div
            key={index}
            className={`${classes.MentionDropdownItem} ${index === selectedIndex ? classes.selectedItem : ''}`}
            onClick={() => onMentionableSelect(suggestion)}
          >
